// npm install node-fetch para poder usar appis
//Compilar package.json -> Linea: tsc src/scripts/index.ts
//tsc src/scripts/index.ts -w

//Ejercicio 1

import { updateImportEqualsDeclaration } from "../../node_modules/typescript/lib/typescript";
import Corrida from "./empresa";
...
function busqueda(empresa: string = "AEF",descripcionS:string) : (Corrida[] | String){
    let encontrada : Corrida[]=[];
    let i : number = 0;
    while(i < corridasWeb.length) {
        if(corridasWeb[i].empresa == empresa && corridasWeb[i].descripcionServicio == descripcionS)
        {
            encontrada.push(new Corrida(corridasWeb[i].numeroCorrida,corridasWeb[i].descripcionServicio,corridasWeb[i].empresa,
                            corridasWeb[i].descripcionEmpresa,(corridasWeb[i].listaDescuentos !== undefined ? true : false) ) );
        }
        i++;
    }
    ...
    {
        return "No hay corridas disponibles";
    }
}

console.log(corridasWeb);
console.log(busqueda('TCH','PRIMERA SELECT'));
...
import {IUser,User} from "./Usuario";
...
let usuario : User = new User("","","","").encontrado('ricardo.jimenez@axity.com');
